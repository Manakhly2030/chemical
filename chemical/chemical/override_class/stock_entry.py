...
class NewStockEntry(StockController):
    def __init__(self, *args, **kwargs):
        super(NewStockEntry, self).__init__(*args, **kwargs)
    ...
    def before_validate(self):
        ...
        self.validate_fg_completed_quantity()
    def validate(self):
    ...
    def validate_fg_completed_qty(self):
        ...
        for item_code, qty_list in item_wise_qty.items():
            total = flt(round(sum(qty_list), 3))
           
            if (self.fg_completed_qty - total) > 0:
                self.process_loss_qty = flt(self.fg_completed_qty - round(total,3), precision)
                self.process_loss_percentage = flt(self.process_loss_qty * 100 / self.fg_completed_qty)

            if self.process_loss_qty:
                total += flt(round(self.process_loss_qty , 3))
            total = round(total , 3)
    ...
    def get_gl_entries(self, warehouse_account):
        gl_entries = super(NewStockEntry, self).get_gl_entries(warehouse_account)
