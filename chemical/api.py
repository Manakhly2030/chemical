		...
		if self.is_new() and not self.amended_from:
			...
			for row in self.additional_costs:
				if row.description == "Spray drying cost":
					row.expense_account = 'Expenses Included In Valuation - {}'.format(abbr)
					row.qty = self.volume
...
def stock_entry_validate(self, method):
	override_calculate_rate_and_amount(self)
...
def override_calculate_rate_and_amount(self):
...
def cal_rate_for_finished_item(self):
	...
	if is_multiple_finish > 1:
		...
		if self.purpose == 'Manufacture' and self.bom_no:
			for row in self.items:
				if row.t_warehouse:
					finished_list.append({row.item_code:row.real_qty}) #create a list of dict of finished item
			...
			for d in self.items:
				...
				item_map[d.item_code] += flt(d.real_qty)
				...
				if d.t_warehouse:
					...
					if d.item_code == frappe.db.get_value('Work Order',self.work_order,'production_item'):
						d.db_set('basic_amount',flt(flt(self.total_outgoing_value*first_item_ratio*d.real_qty)/flt(100*result[d.item_code])))
						d.db_set('additional_cost',flt(flt(self.total_additional_costs*first_item_ratio*d.real_qty)/flt(100*result[d.item_code])))
					...
					if d.item_code == self.second_item:
						d.db_set('basic_amount',flt(flt(self.total_outgoing_value*self.cost_ratio_of_second_item*d.real_qty)/flt(100*result[d.item_code])))
						d.db_set('additional_cost',flt(flt(self.total_additional_costs*self.cost_ratio_of_second_item*d.real_qty)/flt(100*result[d.item_code])))
						d.db_set('amount',flt(d.basic_amount + d.additional_cost))
						d.db_set('basic_rate',flt(d.basic_amount/ d.qty))
						d.db_set('valuation_rate',flt(d.amount/ d.qty))
